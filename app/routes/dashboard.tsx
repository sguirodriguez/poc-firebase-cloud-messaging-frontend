...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "Dashboard | Warning Guy" },
    {
      name: "description",
      content: "Área do usuário — simulador Firebase Messaging",
    },
  ];
}
...
export default function Dashboard() {
  ...
  /** Toast visível na tela quando chega mensagem em foreground */
  ...
  async function handleEnableNotifications() {
    ...
    if (token) setTokenStatus("Token obtido — envie ao backend");
    else if (newPerm === "denied") setTokenStatus("Notificações bloqueadas. Reative nas configurações do site.");
    else setTokenStatus("Não foi possível obter o token.");
  }
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-amber-950/10 flex flex-col">
      {/* Toast: mensagem recebida em foreground */}
      {toast && (
        <div
      ...
      <header className="relative flex justify-between items-center p-6 border-b border-slate-800/50">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="rounded-xl px-4 py-2 text-sm font-medium text-slate-300 hover:text-white hover:bg-slate-800/50 transition border border-slate-700/50"
          >
            Sair
          </button>
        </div>
      ...
      <main className="relative flex-1 max-w-3xl mx-auto w-full px-6 py-10">
        ...
        <p className="text-slate-400 mb-10">
          Área do simulador de notificações push (FCM).
        </p>
        ...
        <section className="rounded-xl border border-slate-700/50 bg-slate-800/30 p-6">
          ...
          <p className="text-slate-400 text-sm mb-4">Status: <span className="font-medium text-slate-300">{permission ?? "…"}</span></p>
          {permission === "denied" && (
            <p className="text-amber-200/90 text-sm mb-4">Reative em: ícone de cadeado na barra de endereço → Notificações → Permitir.</p>
          )}
          <button
            type="button"
        ...
        <section className="mt-6 rounded-xl border border-slate-700/50 bg-slate-800/30 p-6">
          <h2 className="text-lg font-semibold text-white mb-2">Como saber que a mensagem chegou?</h2>
          <ul className="text-slate-400 text-sm space-y-2 mb-4">
            <li><strong className="text-slate-300">App aberto (foreground):</strong> as mensagens aparecem aqui em &quot;Mensagens recebidas&quot; e no console (F12).</li>
            <li><strong className="text-slate-300">App em background ou aba fechada:</strong> o sistema mostra a notificação (pop-up) do navegador/OS.</li>
          </ul>
          <h3 className="text-sm font-medium text-slate-300 mb-2">Mensagens recebidas (foreground)</h3>
          {messages.length === 0 ? (
            <p className="text-slate-500 text-sm">Nenhuma mensagem ainda. Envie um push pelo Firebase Console ou pelo seu backend.</p>
          ) : (
